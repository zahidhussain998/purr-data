...
int sys_defeatrt, sys_autopatch_yoffset, sys_snaptogrid, sys_autosave_interval = 10, sys_gridsize = 10,
...
void sys_loadpreferences( void)
{
    ...
    if (sys_getpreference("autosave_value", prefbuf, MAXPDSTRING))
    sscanf(prefbuf, "%d", &sys_autosave_interval);
...
void glob_savepreferences(t_pd *dummy)
{
    ...
    sprintf(buf1, "%d", sys_autosave_interval);
    sys_putpreference("autosave_value", buf1);
